...
# (cases.txt and cases_non_residential.txt) and generates by-ZIP and by-court #
# summaries of the filings, plaintiff judgments, and default judgments        #
# - Two kinds of summaries are exported:                                      #
#   - Long summaries, with years stacked: by_zip.txt, by_court.txt,           #
#       by_zip_residential_only.txt, and by_court_residential_only.txt        #
#   - Wide summaries, with years spread: by_court_wide.txt and                #
#           by_court_residential_only_wide.txt                                #
...
# Checks
if ('cases.txt' %in% dir(data_folder) == F | 'cases_residential_only.txt' %in% dir(data_folder) == F) {
  stop('ensure that cases.txt and cases_residential_only.txt are in the target directory created in clean-eviction-data.R')
}

# Load data
cases <- read.csv(paste0(data_folder, '/cases.txt'))
cases_residential_only <- read.csv(paste0(data_folder, '/cases_residential_only.txt'))
...
sapply(by_export, function(x) write.table(eval(parse(text = x)), file = paste0(summaries_dir, '/', x, '.txt'), row.names = F))

# Export wide by-court files with yearly % change indicated
write.table(by_court_wide, file = paste0(summaries_dir, '/', 'by_court_wide.txt'), row.names = F)
write.table(by_court_residential_only_wide, file = paste0(summaries_dir, '/', 'by_court_residential_only_wide.txt'), row.names = F)