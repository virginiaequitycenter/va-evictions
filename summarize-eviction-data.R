...
# Checks
if ('cases.csv' %in% dir(data_folder) == F | 'cases_residential_only.csv' %in% dir(data_folder) == F) {
  stop('ensure that cases.csv and cases_residential_only.csv are in the target directory created in clean-eviction-data.R')
}

# Load data
cases <- read.csv(paste0(data_folder, '/cases.csv'))
cases_residential_only <- read.csv(paste0(data_folder, '/cases_residential_only.csv'))
